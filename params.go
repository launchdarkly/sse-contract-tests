...
func (c *commandParams) Read(args []string) bool {
	...
	fs.StringVar(&c.outputDockerScriptVersion, "output-docker-script", "",
		"output a script for running the test in Docker (see README)")
...
func (c commandParams) outputDockerScript() {
	...
	cleanupCommand := fmt.Sprintf(
		"docker stop %s >/dev/null 2>&1; docker network rm contract-tests-network >/dev/null 2>&1",
		testServiceHostname)
